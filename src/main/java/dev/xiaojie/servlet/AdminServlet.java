...
public class AdminServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request,
        ...
        if (action == null || action.equals("") || action.equals("index")) {
            ...
            request.setAttribute("fileList", fileList);
            request.getRequestDispatcher("/jsp/admin/all.jsp").forward(request, response);
        } else if (action.equals("myshare")) {
            //跳转到 我的共享
            List<UserFile> fileList = userFileDao.findMySharedFile(user.getId());
            request.setAttribute("fileList", fileList);
            request.getRequestDispatcher("/jsp/admin/myshare.jsp").forward(request, response);

        } else if (action.equals("mydisk")) {
            //跳转到 我的网盘
            List<UserFile> fileList = userFileDao.findFileListByOwnerId(user.getId());
            request.setAttribute("fileList", fileList);
            request.getRequestDispatcher("/jsp/admin/mydisk.jsp").forward(request, response);
            ...
            request.getRequestDispatcher("/jsp/admin/show_user.jsp").forward(request, response);
        } else if (action.equals("edit")) {
            request.getRequestDispatcher("/jsp/admin/edit.jsp").forward(request, response);
